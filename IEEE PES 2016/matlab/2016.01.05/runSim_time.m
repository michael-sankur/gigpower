...
% This is a multiple time step simulation in which DERs are dispatched to
...
for k1 = 1:length(t)
    loads.spuspu(:,k1,:) = loads.spu(:,:,1) + 0.125*sin(4*t(k1)*pi/t(end))*loads.spu(:,:,1) + 0.125*(t(k1)/t(end))*loads.spu(:,:,1);
end
...
for k1 = 1:length(t)
    ...
    [X,Y,nvar,cvx_optval,cvx_status] = V_Balance_Solver_Yapprox_20160105(feeder,loads,controllers);    
    disp(['cvx : ' cvx_status])
    ...
    Yqp = XX(:,1:n); resQP.Y(:,k1,:) = Yqp;
    Vmagqp = sqrt(Yqp); resQP.Vmag(:,k1,:) = Vmagqp;

    Pqp = XX(:,1*n+1:2*n); resQP.P(:,k1,:) = Pqp;
    Qqp = XX(:,2*n+1:3*n); resQP.Q(:,k1,:) = Qqp;
    Sqp = Pqp + j*Qqp; resQP.S(:,k1,:) = Sqp;
    

    uqp = XX(:,3*n+1:4*n); resQP.u(:,k1,:) = uqp;
    vqp = XX(:,4*n+1:5*n); resQP.v(:,k1,:) = vqp;
    wqp = uqp + j*vqp; resQP.w(:,k1,:) = wqp;
    
    controllers.ww(:,k1,:) = wqp;
