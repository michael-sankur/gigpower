function [X,Y,nvar,cvx_optval,cvx_status] = V_Balance_Solver_Yapprox_Losses_20160105(feeder,loads,controllers,sim)
%%
...
% P_m,k - u_m,k - sum_j ( P_j,k ) = p_m,k + Re(Loss_m,k)
% Q_m,k - v_m,k - sum_j ( Q_j,k ) = q_m,k - cap_m,k + Im(Loss_m,k)
...
%             end
            beq = [beq;
                real(spu(ph,k1)*A0(ph,k1)) + real(Lfbs(ph,k1));
                imag(spu(ph,k1)*A0(ph,k1)) - imag(cappu(ph,k1)) + imag(Lfbs(ph,k1))];
        end        
    end        
end
