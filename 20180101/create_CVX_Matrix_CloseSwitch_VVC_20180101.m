function [nvar, Aineq, bineq, Aeq, beq] = create_CVX_Matrix_CloseSwitch_VVC_20180101(network, sim)
%%
...
%             end
            if vvcstate(ph,k1) == 1
                mvvc = (qmaxpu(ph,k1) - qminpu(ph,k1))/(Vmax(ph,k1) - Vmin(ph,k1));
                Aeq(end,(ph-1)*nvar+k1) = Aeq(end,(ph-1)*nvar+k1) - mvvc/2;
                beq(end) = beq(end) + mvvc/2 - mvvc*Vmin(ph,k1) + qminpu(ph,k1);
            end
        end        
    end        
end
