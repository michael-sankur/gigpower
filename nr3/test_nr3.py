...
dss_path = '/home/toanngo/Documents/GitHub/cigar-document/ceds-cigar/pycigar/data/ieee13busdata/ieee13.dss'
...
for i in range(len(times)-1):
    ...
    for k,v in enumerate(all_load_names):
        NR3.set_load_kw(v, kW_list[k]* (1 + 0.1*np.sin(2*np.pi*0.01*times[i+1])))
        NR3.set_load_kvar(v, kvar_list[k] * (1 + 0.1*np.sin(2*np.pi*0.01*times[i+1])))
