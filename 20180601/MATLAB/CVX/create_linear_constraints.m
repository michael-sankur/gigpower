function [nvar, Aineq, bineq, Aeq, beq] = create_linear_constraints(network,slacknode,Vslack,sim)
%%
...
if isempty(sim.VNR)
    VNR = [1*ones(1,nnode);
        ...
        1*exp(j*120*pi/180)*ones(1,nnode)].*NPH;
else
    VNR = sim.VNR;
end
% L_mn
...
for ph = 1:3        
    for k1 = 2:nnode
        if NPH(ph,k1) == 0
            for k2 = 1:size(nodes.inlines,1)
                if nodes.inlines(k2,k1) ~= 0
                    tempE = zeros(1,nnode);
                    tempT = zeros(1,nnode);
                    tempPQ = zeros(1,nline); tempPQ(1,nodes.inlines(k2,k1)) = 1;
                    tempu = zeros(1,nnode);
                    tempv = zeros(1,nnode);                    
                    Aeq = [Aeq;
                        zeros(1,(ph-1)*nvar), tempE tempT tempPQ zeros(1,nline) tempu tempv, zeros(1,(3-ph)*nvar);
            ...
            for k2 = 1:size(nodes.outlines,1)
                if nodes.outlines(k2,k1) ~= 0
                    tempE = zeros(1,nnode);
                    tempT = zeros(1,nnode);
                    tempPQ = zeros(1,nline); tempPQ(1,nodes.outlines(k2,k1)) = 1;
                    tempu = zeros(1,nnode);
                    tempv = zeros(1,nnode);
                    Aeq = [Aeq;
                            zeros(1,(ph-1)*nvar), tempE tempT tempPQ zeros(1,nline) tempu tempv, zeros(1,(3-ph)*nvar);
        ...
        elseif NPH(ph,k1) == 1
            ...
            tempPQ(nodes.inlines(nodes.inlines(:,k1) ~= 0,k1)) = 1;
            tempPQ(nodes.outlines(nodes.outlines(:,k1) ~= 0,k1)) = -1;
...
for k1 = 1:nline
    txnode = TXnum(k1);
    rxnode = TXnum(k1);
    gamman = VNR(:,rxnode)*(1./VNR(:,rxnode)).';
    if NPH(1,txnode) == 0
