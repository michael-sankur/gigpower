function [nvar, Aineq, bineq, Aeq, beq] = create_linear_constraints_base(network,slacknode,Vslack,sim)
%%
...
for k1 = 1:nline
    ...
    rxnode = TXnum(k1);
    gamman = Vref*(1./Vref).';
    if NPH(1,txnode) == 0 || NPH(1,rxnode) == 0 || LPH(ph,k1) == 0
        gamman(1,:) = 0; gamman(:,1) = 0;
    end
    if NPH(2,txnode) == 0 || NPH(2,rxnode) == 0 || LPH(ph,k1) == 0
        gamman(2,:) = 0; gamman(:,2) = 0;
    end
    if NPH(3,txnode) == 0 || NPH(3,rxnode) == 0 || LPH(ph,k1) == 0
...
% If phase phi present at txnode, rxnode and on line (xtnode,rxnode) - 
% Linearized voltage magnitude equation enforced
% -E_m + E_n + 2 M_mn*P_mn + 2 N_mn*Q_mn = 0
% -E_m^phi + E_n^phi + sum_psi ( 2*M_mn^{phi,psi}*P_mn^psi - 2*N_mn^{phi,psi}*Q_mn^psi ) = 0
...
% If phase phi present at txnode, rxnode and on line (xtnode,rxnode) - 
% Linearized voltage angle equation enforced
% -Theta_m + Theta_n - N_mn P_n - M_mn Q_n = 0
% -theta_m^phi + theta_n^phi - sum_psi ( -N_mn^{phi,psi}*P_mn^psi - M_mn^{phi,psi}*Q_mn^psi ) 
...
if isfield(cons,'realpowerdispatch') == 1
    for ph = 1:3
        for k1 = 1:nnode
...
if isfield(cons,'reactivepowerdispatch') == 1
    for ph = 1:3
        for k1 = 1:nnode
...
if isfield(cons,'realgenonly') == 1
    for ph = 1:3
        for k1 = 1:nnode
            if cons.realgenonly(ph,k1) == 1
                tempu = zeros(1,nnode); tempu(1,k1) = 1;
                tempv = zeros(1,nnode);
                Aineq = [Aineq;
                    zeros(1,(ph-1)*nvar), tempE tempT tempP tempQ tempu tempv, zeros(1,(3-ph)*nvar)];
