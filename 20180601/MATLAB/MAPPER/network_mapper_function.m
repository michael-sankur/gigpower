function [network] = network_mapper_function(fp, fn)

% Michael Sankur - msankur@lbl.gov
% 2018.06.01

% This function loads a network configuration and returns a struct
% containing all network information
...
% including the following structs
...
%             loads.type{kph,knode} = temp2{2};
        ...
        if strcmp('real',temp2{1})
            ...
            temp2 = strsplit(temp1{k2+1},'=');
            if strcmp('unit',temp2{1}) && strcmp('pu',temp2{2})
                loads.ppu(kph,knode) = loads.ppu(kph,knode);
            end
            if strcmp('unit',temp2{1}) && strcmp('W',temp2{2})
                loads.ppu(kph,knode) = loads.ppu(kph,knode)*1/Sbase;
            end
            if strcmp('unit',temp2{1}) && strcmp('kW',temp2{2})
                loads.ppu(kph,knode) = loads.ppu(kph,knode)*1e3/Sbase;
            end
            if strcmp('unit',temp2{1}) && strcmp('MW',temp2{2})
        ...
        if strcmp('reac',temp2{1})
            ...
            if strcmp('unit',temp2{1}) && strcmp('VAr',temp2{2})
                loads.qpu(kph,knode) = loads.qpu(kph,knode)*1e0/Sbase;
            end
            if strcmp('unit',temp2{1}) && strcmp('kVAr',temp2{2})
                loads.qpu(kph,knode) = loads.qpu(kph,knode)*1e3/Sbase;
            end
            if strcmp('unit',temp2{1}) && strcmp('MVAr',temp2{2})
...
for k1 = 1:size(controllerlinearray,2)
    ...
    for k2 = 1:length(temp1)    
        ...
        % ES controller probe amplitude
        if strcmp('aes',temp2{1})
            cons.aes(kph,knode) = str2double(temp2{2});
        end
...
for k1 = 1:size(vvclinearray,2)
    ...
    for k2 = 1:length(temp1)    
        ...
        if strcmp('Vmin',temp2{1})
            ...
            if strcmp('unit',temp2{1}) && strcmp('V',temp2{2})
                vvc.Vminpu(kph,knode) = vvc.Vminpu(kph,knode)*1e0/Vbase;
            end
            if strcmp('unit',temp2{1}) && strcmp('kV',temp2{2})
                vvc.Vminpu(kph,knode) = vvc.Vminpu(kph,knode)*1e3/Vbase;
            end
            if strcmp('unit',temp2{1}) && strcmp('MV',temp2{2})
