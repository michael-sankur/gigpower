...
def create_linear_constraints_base_function(network,sim,slacknode,Vslack):
    ...
    for ph in range(0,3):
        for k1 in range(1,nnode):
            if NPH[ph,k1] == 0:
                for k2 in range(0,inlines.shape[0]):
                    if inlines[k2][k1] != -1:
                        tempE = np.zeros((1,nnode))
                        tempT = np.zeros((1,nnode))
                        tempPQ = np.zeros((1,nline))
                        tempPQ[0,inlines[k2][k1]] = 1
                        tempu = np.zeros((1,nnode))
                        tempv = np.zeros((1,nnode))
                ...
                for k2 in range(0,outlines.shape[0]):
                    if outlines[k2][k1] != -1:
                        tempE = np.zeros((1,nnode))
                        tempT = np.zeros((1,nnode))
                        tempPQ = np.zeros((1,nline))
                        tempPQ[0,outlines[k2][k1]] = 1
                        tempu = np.zeros((1,nnode))
                        tempv = np.zeros((1,nnode))
