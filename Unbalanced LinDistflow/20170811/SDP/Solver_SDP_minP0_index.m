...
Y = Y(phidx1(phidx1 ~= 0),phidx1(phidx1 ~= 0))
...
% end
% ea, eb, ec
ea = zeros(max(max(phidx2)),nnode);
eb = zeros(max(max(phidx2)),nnode);
ec = zeros(max(max(phidx2)),nnode);
for k1 = 1:nnode
    if phidx2(1,k1) ~= 0
        ea(phidx2(1,k1),k1) = 1;
    end
...
cvx_begin %sdp
    cvx_solver sedumi;
    variable Xsdp(nph,nph) hermitian;
...
%             end
    ...
    Xsdp(3,2) == V0(3)*V0(2)';
    Xsdp(3,3) == V0(3)*V0(3)';
    Xsdp == hermitian_semidefinite(nph);
%     Xsdp >= 0;
