...
%         kq = length(Qlim);
        ...
        for kmc = 1:100
            ...
            ME(end+1) = max(max(abs(abs(FBS0.V) - abs(FBS1.V))));
            TE(end+1) = max(max(abs(angle(FBS0.V) - angle(FBS1.V))));
            VE(end+1) = max(max(abs(FBS0.V - FBS1.V)));
            PE(end+1) = max(max(abs(real(FBS0.Srx) - real(FBS1.Srx))));
            QE(end+1) = max(max(abs(imag(FBS0.Srx) - imag(FBS1.Srx))));
            SE(end+1) = max(max(abs(FBS0.Srx - FBS1.Srx)));
