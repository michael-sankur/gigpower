function [FBS] = FBS_RX_A5(feeder,nodes,lines,configs,loads,caps,controllers,Vref)

% Linear approximation
% Losses
% Hmn
% Gamma based on RX node voltage
% Voltage magnitudes applied to angle equation
...
while max(abs(Etest - E0)) >= 1e-6 && max(abs(Dtest - D0)) >= 1e-6
    ...
    for k1 = 2:1:nnode
        ...
        for k2 = 1:1:length(lineidx)
            ...
            for ph = 1:3
                if nodes.PH(ph,k1) == 1 && lines.PH(ph,kline) == 1 && nodes.PH(ph,knode) == 1
%             kline = nodes.outmat(k2,k1);
            % calculate voltage at child node
                    E(ph,knode) = E(ph,k1) - 2*(Mmn(ph,:,kline)*P(:,kline) + Nmn(ph,:,kline)*Q(:,kline)) - Hmn(ph,kline);
                    D(ph,knode) = D(ph,k1) - 180/pi*(Nmn(ph,:,kline)*P(:,kline) - Mmn(ph,:,kline)*Q(:,kline))/(abs(V(ph,k1))*abs(V(ph,knode)));
                    E(nodes.PH == 0) = 0;
                end
            end
        end
    ...
    V = sqrt(E).*exp(1j*pi/180*D).*nodes.PH;
    for k2 = 1:nline
        Vmn = lines.PH(:,k2).*V(:,lines.TXnum(k2)) - V(:,lines.RXnum(k2));
        Lmn(:,k2) = Vmn.*conj(lines.FYpu(:,:,k2)*Vmn);
        Hmn(:,k2) = Vmn.*conj(Vmn);
    end
    ...
    for k2 = nnode:-1:2
        ...
        P(:,inline) = real(s(:,k2)) + sum(P(:,outlines),2) + sum(real(Lmn(:,outlines)),2);
        Q(:,inline) = imag(s(:,k2)) + sum(Q(:,outlines),2) + sum(imag(Lmn(:,outlines)),2);
    end
...
%         idx = find(FM(cn,:) == -1);
        ...
        while(flag)
            ...
            % current node
            for ph = 1:3
                if nodes.PH(ph,cn) == 1 && nodes.PH(ph,idx) == 1 && lines.PH(ph,kline) == 1
                   E(ph,idx) = E(ph,cn) + 2*(Mmn(ph,:,kline)*P(:,kline) + Nmn(ph,:,kline)*Q(:,kline)) + Hmn(ph,kline);
                   D(ph,idx) = D(ph,cn) + 180/pi*(Nmn(ph,:,kline)*P(:,kline) - Mmn(ph,:,kline)*Q(:,kline))/(abs(V(ph,idx))*abs(V(ph,cn)));
                   E(nodes.PH == 0) = 0;
                end
            end
...
Srx = P + 1j*Q; Srx(lines.PH == 0) = 0;
Stx = Srx + Lmn; Stx(lines.PH == 0) = 0;
