function [FBS] = FBS(feeder,nodes,lines,configs,loads,caps,controllers,subidx,Vref)
...
for k1 = 1:nodes.nnode
    if sum(inmat(:,k1) ~= 0) == 1 && sum(outmat(:,k1) ~= 0) == 0
        tn = [tn k1];
    end
    if sum(inmat(:,k1) ~= 0) == 1 && sum(outmat(:,k1) ~= 0) == 1
        pn = [pn k1];
    end
    if sum(inmat(:,k1) ~= 0) == 1 && sum(outmat(:,k1) ~= 0) >= 2
        jn = [jn k1];
    end    
end
% rn, pn, jn, tn
...
%         idx = find(FM(cn,:) == -1);
        ...
        while(flag)
            ...
            I(:,inmat(idx)) = I(:,kline) + conj(s(:,idx)./V(:,idx));
            % zero out current for nonexistent phases at all nodes
            I(lines.PH==0) = 0;
...
%         disp('NEW JUNCTION NODE')
        ...
        % find children of current node
%         idx = find(FM(cn,:) == 1);
%         idx = outmat(outmat(:,cn) ~= 0,cn);
        lineidx = outmat(outmat(:,cn) ~= 0,cn);
        for k2 = 1:1:length(lineidx)
...
for k1=1:1:nline
    Stx(:,k1) = V(:,TXnum(k1)).*conj(I(:,k1));
    Srx(:,k1) = V(:,RXnum(k1)).*conj(I(:,k1));
end
