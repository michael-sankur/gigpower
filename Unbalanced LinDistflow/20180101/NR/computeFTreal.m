function FT = computeFTreal(XNR,network,slackidx,slackVnom)
...
FTSUBV = [XNR(2*slackidx-1) - real(slackVnom(1));
    XNR(2*slackidx) - imag(slackVnom(1));
    XNR(2*nnode + 2*slackidx-1) - real(slackVnom(2));
    XNR(2*nnode + 2*slackidx) - imag(slackVnom(2));
    XNR(2*2*nnode + 2*slackidx-1) - real(slackVnom(3));
    XNR(2*2*nnode + 2*slackidx) - imag(slackVnom(3))];
...
for ph = 1:3
    for k1 = 2:nnode
        ...
        elseif NPH(ph,k1) == 1
            ...
            FTKCL(idxre) = FTKCL(idxre) - real(spu(ph,k1))*(APQ(ph,k1) + AZ(ph,k1)*(XNR(idxAm)^2 + XNR(idxBm)^2)) - real(wpu(ph,k1));
            FTKCL(idxim) = FTKCL(idxim) - imag(spu(ph,k1))*(APQ(ph,k1) + AZ(ph,k1)*(XNR(idxAm)^2 + XNR(idxBm)^2)) + cappu(ph,k1) - imag(wpu(ph,k1)) - vvcpu(ph,k1);
