function OPTSOL = Solver_LinDist3Flow_TX_min36_20160603_magangle(feeder, nodes, lines, configs, loads, caps, controllers, sim)
%%
...
%     Z1 = fP0'*X;
%     Z1 = fYA3ABC'*X;
%     Z1 = abs(X(kYa+3) - X(kYa+6)) + abs(X(kYb+3) - X(kYb+6)) + abs(X(kYc+3) - X(kYc+6));
%     Z1 = norm(X(kYa+3)-X(kYa+6),1) + norm(X(kYb+3)-X(kYb+6),1) + norm(X(kYc+3)-X(kYc+6),1);
    ZY = (X(kYa+3) - X(kYa+6))^2 + (X(kYb+3) - X(kYb+6))^2 + (X(kYc+3) - X(kYc+6))^2;
    ZD = (X(kDa+3) - X(kDa+6))^2 + (X(kDb+3) - X(kDb+6))^2 + (X(kDc+3) - X(kDc+6))^2;
%     Z1 = norm(X(kYa+3)-X(kYa+6),2) + norm(X(kYb+3)-X(kYb+6),2) + norm(X(kYc+3)-X(kYc+6),2);
    ...
    for ph = 1:3
        for k1 = 1:nnode
            ...
            norm(X([ku kv]),2) <= 1.0*wmaxpu(ph,k1)
        end
    end    
cvx_end
