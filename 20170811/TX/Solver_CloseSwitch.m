function OPTSOL = Solver_CloseSwitch(feeder1, nodes1, lines1, configs1, loads1, caps1, controllers1, sim1, ...
    feeder2, nodes2, lines2, configs2, loads2, caps2, controllers2, sim2)
%%
...
if isempty(sim1.Vfbs)
    Vfbs1 = [1*ones(1,nnode1);
        1*exp(j*-120*pi/180)*ones(1,nnode1);
        1*exp(j*120*pi/180)*ones(1,nnode1)].*nodes1.PH;
else
    Vfbs1 = sim1.Vfbs;
end
if isempty(sim1.Lfbs)
    Lfbs1 = zeros(3,nline1);
else
    Lfbs1 = sim1.Lfbs;
end
if isempty(sim1.Hfbs)
    Hfbs1 = zeros(3,nline1);
else
    Hfbs1 = sim1.Hfbs;
end

nvar1 = nnode1 + nnode1 + nline1 + nline1 + nnode1 + nnode1; % number of variables per phase
