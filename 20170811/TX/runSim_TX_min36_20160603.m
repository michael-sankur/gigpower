...
loads.spu(2,:) = 1.5*loads.spu(2,:);
loads.spu(3,:) = 1.25*loads.spu(3,:);
...
% OPTSOL = Solver_LinDist3Flow_TX_minP0_20160603_magonly(feeder, nodes, lines, configs, loads, caps, controllers, sim)
% 
% nvar = OPTSOL.nvar;
% Xa = OPTSOL.X(1:nvar);
...
Vopt = Vopt.*exp(j*pi/180*Dopt).*nodes.PH
...
demopt = loads.spu.*(loads.aPQ + loads.aZ.*Yopt).*nodes.PH
