function [optsolmin, optsolmax] = Solver_LinDist3Flow_TX_Feasible_Eminmax(feeder, nodes, lines, configs, loads, caps, controllers, sim)
%%
...
for ph = 1:3
    for k1 = 2:nnode
        if nodes.PH(ph,k1) == 0
            ...
            tempv = zeros(1,nnode);
            Aeq = [Aeq;
                zeros(1,(ph-1)*nvar), tempY tempDelta tempP tempQ tempu tempv, zeros(1,(3-ph)*nvar)];
            beq = [beq;
                0];
        elseif nodes.PH(ph,k1) == 1
            tempY = zeros(2,nnode); tempY(:,k1) = [1; -1];
            tempDelta = zeros(2,nnode);
            tempP = zeros(2,nline);
            tempQ = zeros(2,nline);
            tempu = zeros(2,nnode);
            tempv = zeros(2,nnode);
            Aineq = [Aineq;
                zeros(2,(ph-1)*nvar), tempY tempDelta tempP tempQ tempu tempv, zeros(2,(3-ph)*nvar)];
            bineq = [bineq;
                1.05^2;
                -(0.95^2)];
        end
...
tempY = zeros(1,nnode); tempY(1,slacknode) = 1;
tempDelta = zeros(1,nnode);
tempP = zeros(1,nline);
tempQ = zeros(1,nline);
tempu = zeros(1,nnode);
...
Aeq = [Aeq;
    tempY tempDelta tempP tempQ tempu tempv, zeros(1,nvar), zeros(1,nvar);
    zeros(1,nvar), tempY tempDelta tempP tempQ tempu tempv, zeros(1,nvar);
    zeros(1,nvar), zeros(1,nvar), tempY tempDelta tempP tempQ tempu tempv];
...
for ph = 1:3
    for k1 = 1:nline
        if lines.PH(ph,k1) == 0
            ...
            Aeq = [Aeq;
                zeros(1,(ph-1)*nvar), tempY tempDelta tempPQ zeros(1,nline) tempu tempv, zeros(1,(3-ph)*nvar);
                zeros(1,(ph-1)*nvar), tempY tempDelta zeros(1,nline) tempPQ tempu tempv, zeros(1,(3-ph)*nvar)];
            beq = [beq;
                0;
                0];
        end
...
for ph = 1:3        
    for k1 = 2:nnode
        ...
        elseif nodes.PH(ph,k1) == 1
            ...
            Aeq  = [Aeq;
                zeros(1,(ph-1)*nvar), real(tempY) tempDelta tempPQ zeros(1,nline) tempuv zeros(1,nnode), zeros(1,(3-ph)*nvar);
                zeros(1,(ph-1)*nvar), imag(tempY) tempDelta zeros(1,1*nline) tempPQ zeros(1,nnode) tempuv, zeros(1,(3-ph)*nvar)];
            beq = [beq;
...
for k1 = 1:nline
    ...
    if nodes.PH(1,txnode) == 1 && nodes.PH(1,rxnode) == 1 && lines.PH(1,k1) == 1
        ...
        Aeq = [Aeq;
            ...
            zeros(1,nnode) tempDelta tempPb tempQb tempu tempv, ...
            zeros(1,nnode) tempDelta tempPc tempQc tempu tempv];
        beq = [beq;
            0];
%             Hfbs(1,k1)];
    end      
...
for k1 = 1:nline
    ...
    if nodes.PH(2,txnode) == 1 && nodes.PH(2,rxnode) == 1 && lines.PH(2,k1) == 1
        ...
        Aeq = [Aeq;
            zeros(1,nnode) tempDelta tempPa tempQa tempu tempv, ...
...
for ph = 1:3
    for k1 = 2:nnode
        if nodes.PH(ph,k1) == 0
            tempuv = zeros(1,nnode); tempuv(1,k1) = 1;
            Aeq = [Aeq;
                zeros(1,(ph-1)*nvar), tempY tempDelta tempP tempQ tempuv zeros(1,nnode), zeros(1,(3-ph)*nvar);
                zeros(1,(ph-1)*nvar), tempY tempDelta tempP tempQ zeros(1,nnode) tempuv, zeros(1,(3-ph)*nvar)];
            beq = [beq;
                0;
                0];
        end
...
for ph = 1:3
    for k1 = 1:nnode
        if controllers.wmaxpu(ph,k1) ~= 0
            tempu = zeros(ncirc+1,nnode); tempu(:,k1) = cosd(theta');
            tempv = zeros(ncirc+1,nnode); tempv(:,k1) = sind(theta');
            Aw = [Aw;
                zeros(ncirc+1,(ph-1)*nvar), zeros(ncirc+1,2*nnode + 2*nline) tempu tempv, zeros(ncirc+1,(3-ph)*nvar)];
            bw = [bw;
...
tempE = zeros(1,nnode); tempE(sim.tnode) = 1;
ftemp = [tempE zeros(1,nnode) zeros(1,2*nline) zeros(1,2*nnode)];
fa = [ftemp zeros(1,2*nvar)];
fb = [zeros(1,nvar) ftemp zeros(1,nvar)];
fc = [zeros(1,2*nvar) ftemp];

cvx_begin quiet
    expression Emin;
    variable X(3*nvar);
    Emin = X(nvar*(sim.tph - 1) + sim.tnode);
    minimize(Emin);
    subject to
    Aeq * X == beq;
    Aineq * X <= bineq;
...
optsolmin.nvar = nvar;
optsolmin.Emin = Emin;
optsolmin.Vmin = sqrt(Emin);

cvx_begin quiet
    expression Emax;
    variable X(3*nvar);
    Emax = X(nvar*(sim.tph - 1) + sim.tnode);
    maximize(Emax);
    subject to
    Aeq * X == beq;
    Aineq * X <= bineq;
