function JT = computeJTrealA2(X,feeder,nodes,lines,configs,loads,caps,controllers,emeas,slackidx,slackVnom)
...
%             JTANG(end,idxEn) = 0.5*sqrt(X(idxEm))*(-X(idxDm) + X(idxDn))/sqrt(X(idxEn));
            JTANG(end,idxDm) = -emeas(ph,lines.TXnum(k1))*emeas(ph,lines.RXnum(k1));
            JTANG(end,idxDn) = emeas(ph,lines.TXnum(k1))*emeas(ph,lines.RXnum(k1));            
