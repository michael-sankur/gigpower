function FT = computeFTreal2sub(X,feeder,nodes,lines,configs,loads,caps,controllers,subidx,subVnom)
...
for k1 = 1:length(subidx)
    FTSUBV = [FTSUBV;
        X(2*subidx(k1)-1) - real(subVnom(1,k1));
        X(2*subidx(k1)) - imag(subVnom(1,k1));
        X(2*nnode + 2*subidx(k1)-1) - real(subVnom(2,k1));
        X(2*nnode + 2*subidx(k1)) - imag(subVnom(2,k1));
        X(2*2*nnode + 2*subidx(k1)-1) - real(subVnom(3,k1));
        X(2*2*nnode + 2*subidx(k1)) - imag(subVnom(3,k1))];
end
