function FT = computeFTrealA3(X,feeder,nodes,lines,configs,loads,caps,controllers,slackidx,slackVnom)
...
for ph = 1:3
    for k1 = 1:nline
        ...
        elseif LPH(ph,k1) == 1
            ...
            FTANG(end+1,1) = sqrt(X(idxEm))*sqrt(X(idxEn))*(-X(idxDm) + X(idxDn)) ...
