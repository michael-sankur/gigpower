function [FBS] = FBS_RX_A6(feeder,nodes,lines,configs,loads,caps,controllers,Vref)
...
%             kline = nodes.outmat(k2,k1);
            % calculate voltage at child node
                    ...
                    D(ph,knode) = D(ph,k1) - asind(Nmn(ph,:,kline)*P(:,kline) - Mmn(ph,:,kline)*Q(:,kline))/(abs(V(ph,k1))*abs(V(ph,knode)));
                    E(nodes.PH == 0) = 0;
                end
            end
        end
        ...
        while(flag)
            ...
            for ph = 1:3
                if nodes.PH(ph,cn) == 1 && nodes.PH(ph,idx) == 1 && lines.PH(ph,kline) == 1
                   ...
                   D(ph,idx) = D(ph,cn) + asind(Nmn(ph,:,kline)*P(:,kline) - Mmn(ph,:,kline)*Q(:,kline))/(abs(V(ph,idx))*abs(V(ph,cn)));
                   E(nodes.PH == 0) = 0;
                end
            end
