function [FBS] = FBS_TX_A5(feeder,nodes,lines,configs,loads,caps,controllers,Vref)
...
%             kline = nodes.outmat(k2,k1);
    ...
    for k2 = nnode:-1:2
        ...
        P(:,inline) = real(s(:,k2)) + sum(P(:,outlines),2) + real(Lmn(:,inline));
        Q(:,inline) = imag(s(:,k2)) + sum(Q(:,outlines),2) + imag(Lmn(:,inline));
    end
