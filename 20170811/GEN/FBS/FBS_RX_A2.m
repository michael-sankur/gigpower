function [FBS] = FBS_RX_A2(feeder,nodes,lines,configs,loads,caps,controllers,Vref)
...
%         disp('NEW JUNCTION NODE')
        ...
        V = sqrt(E).*exp(1j*pi/180*D).*nodes.PH;
        for k2 = 1:nline
            Vmn = V(:,lines.TXnum(k2)) - V(:,lines.RXnum(k2));
            Lmn(:,k2) = lines.PH(:,k2).*Vmn.*conj(lines.FYpu(:,:,k2)*Vmn);
        end
