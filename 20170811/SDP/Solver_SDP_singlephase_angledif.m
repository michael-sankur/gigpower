function [sdpsol,SDPMAT] = Solver_SDP_singlephase_angledif(feeder, nodes, lines, configs, loads, caps, controllers, sim)
%%
...
%     trace(PhiRemn*Xsdp)*tand(lb) <= trace(PhiImmn*Xsdp)
    Philb = PhiRemn*sind(lb) - PhiImmn*cosd(lb);
    trace(Philb*Xsdp) <= 0;
    
    
    ub = sim.ub;
%     trace(PhiImmn*Xsdp) <= trace(PhiRemn*Xsdp)*tand(ub)
    Phiub = PhiImmn*cosd(ub) - PhiRemn*sind(ub);
    trace(Phiub*Xsdp) <= 0;
