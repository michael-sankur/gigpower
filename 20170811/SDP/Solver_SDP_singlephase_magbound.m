function [sdpsol,SDPMAT] = Solver_SDP_singlephase_magbound(feeder, nodes, lines, configs, loads, caps, controllers, sim)
%%
...
cvx_begin sdp
    ...
    for k1 = 2:nnode
        Zu = Zu + trace(PhiQa(:,:,k1)*Xsdp) - aPQ(1,k1)*real(spu(1,k1));
    end

    for k1 = 2:nnode
        Zv = Zv - eps*(trace(PhiQa(:,:,k1)*Xsdp) - aPQ(1,k1)*imag(spu(1,k1)) + cappu(1,k1));
    end
%     Zsdp = gammasdp;
    minimize(1*Zu + 1*Zv);
%     minimize(alphasdp)
...
%     trace(PhiVa(:,:,tn)*Xsdp) - vref^2 <= gammasdp;
%     -trace(PhiVa(:,:,tn)*Xsdp) + vref^2 <= gammasdp;

    % Magnitude Difference Minimization
    
%     trace(PhiVa(:,:,tn1)*Xsdp) - trace(PhiVa(:,:,tn2)*Xsdp) <= gammasdp;
%     -trace(PhiVa(:,:,tn1)*Xsdp) + trace(PhiVa(:,:,tn2)*Xsdp) <= gammasdp;
    
%     [gammasdp (trace(PhiVa(:,:,tn1)*Xsdp) - trace(PhiVa(:,:,tn2)*Xsdp));
%         (trace(PhiVa(:,:,tn1)*Xsdp) - trace(PhiVa(:,:,tn2)*Xsdp)) 1] >= 0;

    % Magnitude Difference Bounds
    
    sim.lb <= trace((PhiVa(:,:,tn1) - PhiVa(:,:,tn2))*Xsdp)
    trace((PhiVa(:,:,tn1) - PhiVa(:,:,tn2))*Xsdp) <= sim.ub;
